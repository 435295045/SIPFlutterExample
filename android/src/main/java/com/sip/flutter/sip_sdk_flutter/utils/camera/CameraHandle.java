...
public class CameraHandle {
            ...
            for (String cameraId : cameraManager.getCameraIdList()) {
                ...
                if (facing != null) {
                    ...
                    if (map != null) {
                        ...
                        previewSize = selectResolution(sizes, CAMERA_IMAGE_WIDTH, CAMERA_IMAGE_HEIGHT);
                    }
                    if (previewSize != null) {
                        ...
                        // 默认前置摄像头
                        if (CameraCharacteristics.LENS_FACING_FRONT == cameraInfo.facing) {
    ...
    public void open() {
        ...
        try {
            //先关闭一下
            close();
            if (currentCameraInfo == null && !targetResolutionMap.isEmpty()) {
                Map.Entry<String, CameraInfo> firstEntry = targetResolutionMap.entrySet().iterator().next();
                currentCameraInfo = firstEntry.getValue();
            }
            if (currentCameraInfo == null) {
                return;
            }
            setupImageReader(currentCameraInfo.previewSize);
    ...
    public void cameraSwitch() {
        if (currentCameraInfo != null && !targetResolutionMap.isEmpty()) {
            for (Map.Entry<String, CameraInfo> entry : targetResolutionMap.entrySet()) {
                ...
                if (!Objects.equals(value.cameraId, currentCameraInfo.cameraId) && !Objects.equals(value.facing, currentCameraInfo.facing)) {
